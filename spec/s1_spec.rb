...
RSpec.describe S1 do
  describe ".to_state" do
    it "prepares anything into a State, via #to_s1 when the object has one" do
      S1.config.provider = S1::Providers::Stub.new(noul: 0.9)
      expect(S1.to_state("Michael")).to be_a(S1::State)
      expect(S1.to_state("x", threshold: 0.95).judge?("q?")).to be(false)
      custom = Class.new { def to_s1(**) = S1::State.new("converted", **) }.new
      expect(S1.to_state(custom).rendered).to eq("converted")
