...
RSpec.describe "S1 provider conformance" do
  ...
  describe "a provider whose legend is not keyed by rank" do
    it_behaves_like "an S1 provider", lambda {
      S1::Providers::Stub.new(yes: 0.8, which: :b,
                              how: { legend: { "5" => "low", "6" => "mid", "7" => "high" },
                                     probabilities: { "5" => 0.2, "6" => 0.6, "7" => 0.2 }, confidence: 0.6 })
    }
  end
  ...
  let(:surfaces) do
    {
      escalate: %i[to_f true? false? confident? probabilities confidence id type kind scale <=> coerce],
      department: %i[to_s to_sym [] == categories confident? probabilities confidence id type kind scale],
      severity: %i[to_f expectation key index level levels confident? probabilities confidence id type kind scale]
    }
  end
  ...
  it "answers the same ids in the same order" do
    expect(stub_result.distributions.keys).to eq(typesafe_result.distributions.keys)
    expect(stub_result.distributions.keys).to eq(%i[escalate department severity])
  end

  it "yields identical distribution classes per id" do
  ...
  it "keeps type-specific methods off the other types" do
    [stub_result, typesafe_result].each do |result|
      ...
      expect(result[:severity]).not_to respond_to(:legend)
    end
  end

  it "agrees on every scale" do
    [stub_result, typesafe_result].each do |result|
      expect(result[:escalate].scale).to eq([true, false])
      expect(result[:department].scale).to eq(S1.scale(returns: nil, billing: nil, shipping: nil))
      expect(result[:department].scale.to_a).to eq(%i[returns billing shipping])
      expect(result[:severity].scale).to eq(S1.scale("Cosmetic", "Degraded", "Blocking"))
      expect(result[:severity].scale.labels).to eq(%w[Cosmetic Degraded Blocking])
    end
  end
  ...
  it "agrees on the headline reading of each answer" do
    ...
    expect(stub_result[:severity].key).to eq(typesafe_result[:severity].key)
    expect(stub_result[:severity].levels).to eq(typesafe_result[:severity].levels)
    expect(stub_result[:severity].levels.map(&:position)).to eq(typesafe_result[:severity].levels.map(&:position))
  end
  ...
  it "normalizes both through the same rail" do
    ...
    via_typesafe = S1::State.new("text", provider: :typesafe).judge("Human?")
    via_stub = S1::State.new("text", provider: S1::Providers::Stub.new(noul: 0.7)).judge("Human?")
