...
RSpec.describe S1::Providers::TypeSafe do
  ...
  let(:questions) do
    S1::Questions.coerce do |q|
      q.judge  :escalate, "Human?", true: "asks for a person", false: "does not"
  ...
  it "reads its model back: the one given, the configured one, else DEFAULT_MODEL" do
    expect(described_class.new(model: "jev-1").model).to eq("jev-1")
    S1.config.typesafe.model = "jev-2"
    expect(described_class.new.model).to eq("jev-2")
    S1.config.typesafe.model = nil
    expect(described_class.new.model).to eq(described_class::DEFAULT_MODEL)
    expect(S1::Providers::Protocol.new(model: "local").model).to eq("local")
    expect(S1::Providers::Cua.new(checkpoint: "ckpt").model).to eq("ckpt")
    expect(S1::Providers::Cua.new.model).to eq(S1::Providers::Cua::MODEL)
  end
  ...
  describe "request" do
    ...
    it "omits criteria for a bare noul" do
      ...
      bare = S1::Questions.coerce { |q| q.judge :escalate, "Human?" }
    ...
    it "passes structured instructions and structured state through" do
      ...
      qs = S1::Questions.coerce { |q| q.judge :escalate, structured }
