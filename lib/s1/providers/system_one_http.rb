...
module S1
  module Providers
    # The System One HTTP contract, as jev defined it. A provider includes this
    # and supplies @base_url, @api_key (may be empty), @model, @max_retries,
    # @logger, and optionally @path — see TypeSafe and Protocol. Stdlib only.
    #
    #   POST {base_url}{path}     path defaults to /v1/systemone
    ...
    module SystemOneHTTP
      ...
      # The model this instance sends when the Request names none: its identity, beside its class.
      attr_reader :model
      ...
      def post(body, timeout:)
        ...
        @logger&.debug { "[s1:#{name}] POST #{uri} (#{body.bytesize} bytes)" }
        http.request(req)
      ...
      def endpoint
        URI.join(@base_url, @path.to_s.empty? ? PATH : @path)
      end
      ...
      def with_retries
        ...
        begin
          ...
          @logger&.warn { "[s1:#{name}] #{e.class} — retry #{attempt}/#{@max_retries} in #{delay}s" }
          sleep(delay)
          retry
        end
      end
      ...
      def parse(request, response)
        ...
        distributions = request.questions.to_h do |id, question|
          ...
          [id, distribution(id, question, raw: a, **fields_from(question, a))]
        end
        build_result(distributions: distributions, model: payload["model"], usage: payload.fetch("usage", {}), raw: payload)
      ...
      def fields_from(question, a)
        ...
        when Question::Score # a server that sends no legend gets one from the question's levels; its `score` stays in raw
          { legend: a["legend"] || question.levels.each_with_index.to_h { |l, i| [i.to_s, l] },
